...
Proof.
  assert (EqDec:EqDec (positive * positive) eq). {
    hnf; intros.
...
Section ParmovSourceSet.
  ...
  Lemma dtransitions_dst_set t st st'
    : dtransitions var t st st' ->
      st_source_set st' ⊆ st_source_set st.
  Proof.
    intros TR.
...
Fixpoint lower (p:inf_partition var) (preg:var) (DL:〔⦃var⦄ * params〕) s (an:ann (set var))
  ...
  match s, an with
    ...
    | stmtApp l Y, ann0 lv  =>
      ...
      let r := least_fresh_P (part_1 p) (lv' \ of_list Z ∪ lv) in
...
Lemma parmov_repl_agree lv (E:onv val) (pmtmp reg lstmp:var) (Y Z:params) (Len:❬Y❭=❬Z❭) (NEQ:reg =/= lstmp) (NEQ2:pmtmp =/= lstmp)
...
Proof.
  ...
  - unfold Parmov.parmove2.
    ...
    + invc H2; clear e.
      ...
      * eapply of_list_get_first in i; dcr. symmetry in H5; invc H5.
        exploit (@update_with_list_lookup_in_list_first _ _ _ E x Z (lookup_list E Y)); dcr;
          eauto with len.
        rewrite H9. rewrite lookup_list_map in H8. inv_get. rewrite <- EQ.
        exploit repl_get_eq; try eapply H2; eauto.
        exploit (@update_with_list_lookup_in_list_first _ _ _ (E [lstmp <- E reg]) x ZR (lookup_list (E [lstmp <- E reg]) YR)); dcr;
          eauto with len.
        -- intros. intro. invc H6.
           unfold ZR in H5. unfold repl in H5. inv_get. cases in EQ0.
           ++ eapply H7. eauto. eauto. eauto.
           ++ subst.
             eapply get_in_of_list in H5. cset_tac.
        -- rewrite H9.
           rewrite lookup_list_map in H6. subst YR. unfold repl in H6.
           inv_get. cases in H4.
           ++ invc COND.
             rewrite <- H4. lud. cset_tac.
           ++ unfold repl in H3. inv_get.
             cases in H3.
             rewrite <- H4. lud. invc H6.
             eapply get_in_of_list in H8. exfalso. cset_tac.
  ...
  - rewrite fst_combine, snd_combine; eauto with len.
    ...
    + eapply of_list_get_first in i; dcr. symmetry in H5; invc H5.
      exploit (@update_with_list_lookup_in_list_first _ _ _ E x0 Z (lookup_list E Y)); dcr;
        eauto with len.
      rewrite H9.
      rewrite lookup_list_map in H8. inv_get. rewrite <- EQ.
      ...
      * subst ZR. intros. unfold repl in *. inv_get.
        ...
        intro. invc H5.
        cset_tac.
      * subst YR. rewrite lookup_list_map in H10.
        unfold repl in H5, H10. inv_get.
        repeat cases; eauto.
        cases in H6.
        -- invc COND.
           rewrite <- H6. lud; eauto.
        -- rewrite <- H6. lud; eauto. invc H10. clear e.
           exfalso. clear - H8 NOTIN.
           eapply get_in_of_list in H8.
           cset_tac'.
    + rewrite !lookup_set_update_not_in_Z; eauto.
      lud. cset_tac.
...
Proof.
  ...
  - rewrite nth_default_eq. erewrite get_nth; eauto using zip_get; simpl.
    ...
    + set (reg:=least_fresh_P (part_1 p) (blv \ of_list Z0 ∪ lv)) in *.
      ...
      assert (NEQ1:reg =/= lstmp). {
        subst reg lstmp.
        intro.
        eapply (part_disj p (least_fresh_P (part_1 p) (blv \ of_list Z0 ∪ lv))); eauto.
      ...
      exploit omap_op_eval_live_agree; try eassumption.
      {
        ...
        - exploit (@list_to_stmt_correct
                     (Parmov.parmove2 positive var_dec (fun _ : positive => pmtmp) Y' Z0)
                     (stmtApp l nil) E' L'). {
            unfold Parmov.parmove2.
            simpl.
          ...
          * right; eapply (pmSim_sim p preg); try eapply LA1; eauto; simpl.
            ...
            -- assert (getAnn al ⊆ blv) by eauto with cset.
               ...
               ++ rewrite fst_combine, snd_combine; eauto.
                 ...
                 rewrite H9. clear - H5. cset_tac.
               ++ clear - H9 H5. cset_tac.
        ...
        - exploit (@list_to_stmt_correct
            ...
            edestruct (least_fresh_P_full_spec (part_1 p) (blv \ of_list Z0 ∪ lv)); dcr.
            exploit (H10 (proj1_sig lreg)).
            - subst lreg. destr_sig. eauto.
            - subst lreg. destr_sig; dcr. simpl in *; dcr.
              exploit (H12 (least_fresh_P (part_1 p) (blv \ of_list Z0 ∪ lv))); eauto.
              + destruct H13; eauto. invc H13. subst reg.
                set (reg:=least_fresh_P (part_1 p) (blv \ of_list Z0 ∪ lv)) in *.
                exfalso. eapply NOTCOND.
                eapply Pos.lt_eq_cases. eapply H12. eauto.
            - rewrite of_list_repl_incl.
              ...
              assert (IN:proj1_sig lreg ∈ lv). {
                rewrite filter_incl in H9. rewrite H1 in H9.
                clear - H9; cset_tac. eauto.
              }
              clear - IN. cset_tac.
          }
          pfold. eapply SimSilent.
          ...
          * right; eapply pmSim_sim; try eapply LA1; eauto; simpl.
            ...
            -- assert (getAnn al ⊆ blv) by eauto with cset.
               symmetry. etransitivity. {
                 ...
                 instantiate (1:=blv).
                 rewrite H9. clear - H5 H6. cset_tac.
               }
               eapply agree_on_incl.
               etransitivity.
               eapply (@parmov_update_with_list (getAnn al));
