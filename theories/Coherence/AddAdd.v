...
Definition ogetdef X (d:X) (o:option X) :=
  match o with
  | Some x => x
  | None => d
  end.
...
Definition addAdd' s := (fun (DL:set var) AP => ((s ∩ DL) ∪ oget AP)).
...
Proof.
  ...
  - simpl. econstructor.
    + destruct y,y0; simpl; try now econstructor.
      * econstructor. inv pf. unfold addAdd'; simpl.
        clear - H3; cset_tac.
    + eauto.
Qed.
