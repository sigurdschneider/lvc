...
(* Remark : adapted to typeclasses by S. Lescuyer, not fully  *)
(* functional yet.                                            *)
(**************************************************************)
...
(** [if t then t1 else t2] executes [t] and, if it does not
  ...
  | eq_Prop : forall (S : Set) (x y : S),
    FSet_elt_Prop (x = y)
  | eq_elt_prop : forall `{OrderedType A} (x y : A),
    FSet_elt_Prop (x === y)
  | neq_elt_prop : forall `{OrderedType A} (x y : A),
    FSet_elt_Prop (x =/= y)
  | In_elt_prop : forall `{F : @FSet A HA} (x : A) (s : set A),
  ...
  | elt_FSet_Prop : forall P,
    FSet_elt_Prop P ->
    FSet_Prop P
  | Empty_FSet_Prop : forall `{F : @FSet A HA} (s : set A),
    FSet_Prop (Empty s)
  | Subset_FSet_Prop : forall `{F : @FSet A HA} (s1 s2 : set A),
    FSet_Prop (Subset s1 s2)
  | Equal_FSet_Prop : forall `{F : @FSet A HA} (s1 s2 : set A),
  ...
  (** [In] is decidable. *)
  Lemma dec_In :
    forall `{HF : @FSetSpecs A HA F} x s, decidable (In x s).
  Proof.
    red; intros; generalize (mem_iff s x); case (mem x s); intuition.
  Qed.

  (** [E.eq] is decidable. *)
  Lemma dec_eq : forall `{HA : OrderedType A} (x y : A), decidable (x === y).
  Proof.
...
(** We now want to get rid of all uses of [=] in favor of
   ...
   work properly.  *)
  (* change_to_E_t;  *)E_eq_to_Logic_eq; subst++;
...
(*   Section Tests. *)
(*   Context `{FSpec : @FSetSpecs elt Helt F}. *)
(*   Implicit Types w x y z : elt. *)
(*   Implicit Types r s : set elt. *)
...
(*     forall (f : set elt -> set elt), *)
(*     forall (g : elt -> elt), *)
(*     forall (s1 s2 : set elt), *)
(*     forall (x1 x2 : elt), *)
(*       Equal s1 (f s2) -> *)
(*       x1 === (g (g x2)) -> *)
(*       In x1 s1 -> *)
